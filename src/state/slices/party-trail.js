...
let partySubject = null;
let trailSubject = null;
let followerCountSubject = null;
...
function normaliseParty(value) {
  ...
  return value.map((member) => {
    ...
    return cloneStructured(member);
  });
}

function normaliseTrail(value) {
  if (!Array.isArray(value)) {
    return [];
  }
  return value.map((step) => {
    ...
    return cloneStructured(step);
  });
}

function ensurePartySignal(initialValue = []) {
  return reactiveContext.ensureSignal(PARTY_SIGNAL_KEY, normaliseParty(initialValue));
}

function ensureTrailSignal(initialValue = []) {
...
export function getPartyValue(fallback = []) {
  return normaliseParty(ensurePartySignal(fallback).value);
}

export function partyStream(initialValue = []) {
...
export function updatePartyValue(value, options = {}) {
...
export function mutatePartyValue(mutator, options = {}) {
  const current = normaliseParty(ensurePartySignal([]).value);
  const next = typeof mutator === 'function' ? mutator(cloneStructured(current)) : current;
  return updatePartyValue(typeof next === 'undefined' ? current : next, options);
}

export function getTrailValue(fallback = []) {
  return normaliseTrail(ensureTrailSignal(fallback).value);
}

export function trailStream(initialValue = []) {
...
export function updateTrailValue(value, options = {}) {
...
export function mutateTrailValue(mutator, options = {}) {
...
export function updateFollowerCountValue(value, options = {}) {
