...
font.init = function*(surf) {
  ...
  glyphBuf = fon.subarray(0x682);
};

font.drawChar = function(ch, pos, color) {
  var x = PAL_X(pos),
      y = PAL_Y(pos);
  // This character does not exist in the font lib.
  if (!(ch in big5Dict));
  // Locate for this character in the font lib.
  var offset = big5Dict[ch] * 30;
  // Draw the character to the surface.
  //y *= surface.pitch;
  for (var i=0; i<30; ++i) {
    var dx = x + ((i & 1) << 3);
    for (var j=0; j<8; ++j) {
      if (glyphBuf[i + offset] & (1 << (7 - j))) {
        // ((LPBYTE)(lpSurface->pixels))[y + dx] = bColor;
        surface.setPixel(dx, y, color);
      }
      dx++;
    }
    //y += (i & 1) * surface.pitch;
...
font.drawASCIIChar = function(ch, pos, color) {
  var x = PAL_X(pos),
      y = PAL_Y(pos),
      offset = (ch & ~128) * 15;
  // Draw the character to the surface.
